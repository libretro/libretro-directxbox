...
void HostBridge::Reset() {
}


uint32_t HostBridge::MMIORead(int barIndex, uint32_t addr, unsigned size) {
    Log(LogLevel::Warning, "HostBridge::MMIORead:   Unimplemented!  bar = %d,  address = 0x%x,  size = %u\n", barIndex, addr, size);
    
    return 0;
}

void HostBridge::MMIOWrite(int barIndex, uint32_t addr, uint32_t value, unsigned size) {
    Log(LogLevel::Warning, "HostBridge::MMIOWrite:  Unimplemented!  bar = %d,  address = 0x%x,  value = 0x%x,  size = %u\n", barIndex, addr, value, size);
}
