...
uint8_t SMC::ReadByte(uint8_t command)
{
   switch (command)
   {
      case SMC_COMMAND_VERSION: /* 0x01 PIC version string */
         switch (m_revision)
         {
            case SCMRevision::P01:
               buffer[1] = "P01"[m_PICVersionStringIndex];
               break;
            case SCMRevision::P2L:
               buffer[1] = "P05"[m_PICVersionStringIndex];
               break; 
            case SCMRevision::D01:
               buffer[1] = "DXB"[m_PICVersionStringIndex];
               break;
            case SCMRevision::D05:
               buffer[1] = "D05"[m_PICVersionStringIndex];
               break;
         }

         m_PICVersionStringIndex = (m_PICVersionStringIndex + 1) % 3;
         break;
      case SMC_COMMAND_AV_PACK:
         return 0x06; /* TODO: Don't hard-code this */
      case SMC_COMMAND_CHALLENGE_1C: 
         return 0x52;
      case SMC_COMMAND_CHALLENGE_1D:
         return 0x72;
      case SMC_COMMAND_CHALLENGE_1E:
         return 0xEA;
      case SMC_COMMAND_CHALLENGE_1F:
         return 0x46;
      default:
